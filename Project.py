    ...
    for keyword in ['√', 'square root of', 'square root', 'square of', 'square', 'punca kuasa dua']:
        if keyword in method:
            ...
            try:                
                ...
                with open('calculate_history.csv', 'a', newline='', encoding='utf-8-sig') as file:
    ...
    for keyword in ['%', 'percent', 'percentage', 'peratus', 'peratusan', 'daripada']:
        if keyword in method:
            try:
                ...
                user_input = f"{num}% ="
                today = datetime.now().strftime("%#m/%#d/%Y")
                with open('calculate_history.csv', 'a', newline='') as file:
                    csv.writer(file).writerow([user_input, result, today])
...
def ZeroCalculator(method):
    ...
    # --- History ---
    if any(kw in method for kw in ['open', 'calculate history', 'history' 'buka', 'pengiraan sejarah']):
        try:
            try:
                history = pd.read_csv('calculate_history.csv', encoding='utf-8')
            except:
                history = pd.read_csv('calculate_history.csv', encoding='latin-1')
    ...
    if any(kw in method for kw in ['clear', 'delete', 'history', 'bersihkan','hapus', 'sejarah']):
        try:
            with open('calculate_history.csv', 'w') as file:
